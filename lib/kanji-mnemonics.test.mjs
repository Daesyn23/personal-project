...
test("every lesson kanji has its own complete memory story", () => {
  ...
  for (const character of characters) {
    ...
    assert.ok(mnemonic.scene.trim());
    assert.ok(!("cues" in mnemonic), "Emoji cues have been replaced by stroke drawings");
...
test("generated content matches the editable originals and stays separate from PDF imports", () => {
  ...
  for (const line of original.split(/\r?\n/)) {
    ...
    const [character, scene, story] = line.split("|").map(field => field.trim());
    parsed[character] = { scene, story };
  }
